...
interface YagmurProps {
  ...
  yaprakSayaci: number;
  saatIndirim: number;
  onUpdate: (data: { som?: number; hak?: number; yaprak_sayaci?: number; saat_indirim?: number }) => void;
}

type DropTur = "som" | "deyis" | "yaprak" | "saat";
...
export function Yagmur({ token, som, hak, yaprakSayaci: yaprak, saatIndirim, onUpdate }: YagmurProps) {
  ...
  const handleClick = useCallback(async (drop: Drop) => {
    ...
    try {
      ...
      else if (drop.tur === "deyis") text = data.deyis || "📖";
  ...
  return (
    <div className="relative min-h-[280px] overflow-hidden rounded-sm">
      <div className="flex gap-5 mb-2 text-xs text-moon-cream/50">
      ...
      <div className="text-[10px] text-moon-cream/30 mb-3 leading-relaxed">
        Üstten düşen emojilere tıkla, ödülleri topla! Yapraklar 10'da +1 hak, saatler -1s.
      </div>
